...
## Codex 설정을 Claude로 이관할 때

Codex의 전역 지침과 로컬 스킬을 Claude Code에도 적용하려면 런타임별 설정과 재사용 가능한 지식을 분리한다.

- Codex 전역 지침 `~/.codex/AGENTS.md`는 Claude 전역 `~/.claude/CLAUDE.md`에 직접 길게 넣기보다 `~/.claude/rules/*.md`로 분리하고, `CLAUDE.md`가 해당 rule 파일들을 import하게 한다.
- `~/.claude/CODEX_AGENTS.md`는 원본 복사본 또는 비교용으로 둘 수 있지만, 실제 활성 경로는 `~/.claude/rules/codex-global.md`처럼 rule 파일로 둔다.
- 사용자 정의 스킬은 `~/.codex/skills/<skill-name>/SKILL.md` 구조를 유지해 `~/.claude/skills/<skill-name>/SKILL.md`로 옮긴다.
- Codex 시스템 스킬, 플러그인, MCP, 승인 정책, 모델 설정은 Claude와 1:1 호환으로 보지 않는다. 참고용 파일로 보관하되 활성 규칙처럼 취급하지 않는다.
- 스킬 본문이 Codex 전용 도구나 directive를 언급하면 Claude Code에서 가능한 도구로 의도만 옮긴다.
- 이관 후 검증 기준은 `~/.claude/CLAUDE.md`의 rule import, `~/.claude/rules/*.md`, `~/.claude/skills/*/SKILL.md`, 기존 `~/.claude/settings.json` 보존 여부를 확인하는 것이다.
