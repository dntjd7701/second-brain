...
이 문서는 Voice to EMR 경과기록지 1차 PoC를 어떤 순서로 구현할지 정리한다. 초기 목표는 OpenAI나 Azure 같은 유료 AI 연동이 아니라, 먼저 경과기록지 화면 안에서 음성 기반 초안 입력 흐름이 자연스럽게 동작하는지 검증하는 것이다. 다만 Web Speech API의 STT 품질 한계가 확인되면 테스트용 음성에 한해 OpenAI STT/SOAP provider를 붙여 흐름을 보강한다.
...
1차에서는 `WebSpeechSttProvider`, `MediaRecorderAiSttProvider`, `LlmSoapNormalizer`, `SoapHtmlRenderer`를 사용한다.
