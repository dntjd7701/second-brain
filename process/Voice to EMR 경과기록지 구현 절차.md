...
```text
1차 STT: OneAI oai001A02 multipart STT
1차 SOAP: OneAI oai001A04 동기 LLM 호출
품질 gate: medical BE에서 관리
FE 계약: 기존 jobId polling 유지
후속: audio seek, GCP Voice Object rawResult, job 복구는 별도 phase
```

10분 이하 녹음은 우선 multipart STT와 기존 polling 구조로 검증한다. GCS batch 기반 Voice Object 흐름은 timeout, rawResult 보존, audio seek, 서버 재시작 후 복구가 실제 요구로 올라올 때 재검토한다.
