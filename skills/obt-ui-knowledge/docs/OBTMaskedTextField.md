...
```tsx
import { OBTMaskedTextField, OBTMaskedTextFieldProps, OBTMaskedTextFieldMethods } from 'luna-orbit';

export interface OBTMaskedTextFieldProps {
  /**
   */
  type: MaskType;
  ...
  /**
   */
  onGetPrivacy?: (e: GetPrivacyEventArgs) => Promise<void>;

}

export interface OBTMaskedTextFieldMethods {
  /**
   */
  createPattern(separator: RegExp, rawExtractor: (value?: string) => string, formatter: (value?: string, options?: { withMasking?: boolean; }) => string, validator: (value?: string) => boolean): (Anonymous class);
  ...
  /**
   */
  getPrivacyValue(): Promise<string>;
...
import { MaskType } from 'luna-orbit/Common/Patterns';
...
import { PrivacyBehaviorEnum } from 'luna-orbit/Common/CommonProps';
export enum PrivacyBehaviorEnum {
     ...
     * 버튼 클릭시 조회
     */
    viewByButton = 'viewByButton',
    /**
     * 포커스시 조회
     */
    viewByFocus = 'viewByFocus',
    /**
     * 항상조회
     */
    always = 'always'
