...
```tsx
import { OBTDropDownList, OBTDropDownListProps, OBTDropDownListMethods } from 'luna-orbit';

export interface OBTDropDownListProps {
  /**
   * 컴포넌트 입력필드에서 보여지는 value값의 리스트를 정의하는 속성
   */
  list: any[];
  ...
  /**
   * 컴포넌트에서 데이터소스를 이용하여 매핑 정의하는 속성
   */
  fieldName?: TypeList;

  /**
   * 새롭게 개발된 OBTDropDownList2 컴포넌트로 대체할 때 사용되는 속성 
...
export interface OBTDropDownListMethods {
  /**
   * 컴포넌트에 포커스를 준다
   */
