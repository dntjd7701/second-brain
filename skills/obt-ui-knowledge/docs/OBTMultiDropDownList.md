# OBTMultiDropDownList

OBTNumberField
Prop : { style, disabled, readonly, required, value, placeHolder, type, onFocus, onBlur, onChange, onMoveFocus }

```tsx
import { OBTMultiDropDownList, OBTMultiDropDownListProps, OBTMultiDropDownListMethods } from 'luna-orbit';

export interface OBTMultiDropDownListProps {
  /**
  ...
  /**
   * 리스트의 최대 선택 가능 갯수를 지정할 수 있습니다.
   */
  maxLength?: number;

  /**
   * dropDown영역 className
   */
...
export interface OBTMultiDropDownListMethods {
  /**
   * 컴포넌트에 포커스를 준다
   */
